...
from alpamayo1_5_sft_qwen3_5.trainer import ReasoningVLA_Trainer
from alpamayo1_5_sft_qwen3_5.trainer import TrainingArguments
from alpamayo1_5_sft_qwen3_5.models.sft_base_model import TrainableReasoningVLA
...
def evaluate(cfg: DictConfig) -> None:
    ...
    if cfg.evaluate.get("eval_ckpt", None) is not None:
        ...
        if issubclass(model_cls, TrainableReasoningVLA):
            ...
            # This recipe only builds models via `from_pretrained_vlm` (there is no
            # 10B-style `from_pretrained` config here), so always route eval_ckpt
            # to the Stage-2 checkpoint path.
