#!/bin/bash
#SBATCH --job-name=cm2b_all28_fp16
...
#SBATCH --output=/temp/achahe/alpamayo-recipes/recipes/alpamayo1_5_distill/training/cm2b_2to1_%j.out
#SBATCH --error=/temp/achahe/alpamayo-recipes/recipes/alpamayo1_5_distill/training/cm2b_2to1_%j.err
set -euo pipefail
...
EOS_CKPT="${EOS_CKPT:-$OUT/output_eos_cotrain_2bmix_all28_lr1x_nav_framecache/checkpoint-6876}"
SMOKE="${SMOKE:-0}"
RUN_OUT="${OUTPUT_DIR:-$OUT/output_cd_eos2bmix_all28_consistency2to1_fp16_master32_2cam_nav}"
if [[ "$SMOKE" == "1" ]]; then
    RUN_OUT="${OUTPUT_DIR:-${RUN_OUT}_smoke_${SLURM_JOB_ID:-$$}}"
    export WANDB_MODE=disabled
fi
if [[ -n "${RESUME:-}" ]]; then
    echo "Exact EMA/online resume is not supported; start from a fresh directory" >&2
    exit 1
fi
if [[ ! -f "$EOS_CKPT/model.safetensors.index.json" || ! -f "$EOS_CKPT/config.json" ]]; then
    echo "Missing sharded EoS checkpoint or config: $EOS_CKPT" >&2
    exit 1
fi
if [[ -d "$RUN_OUT" && -n "$(find "$RUN_OUT" -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
    echo "Refusing non-empty output: $RUN_OUT" >&2
    exit 1
fi
...
EXTRA=("model.eos_checkpoint_path=$EOS_CKPT" "paths.output_dir=$RUN_OUT"
...
echo "[slurm] 2B all-layer cotrained EoS -> two-step-to-one-step consistency"
echo "[slurm] init=$EOS_CKPT; frozen VLM + 28->36 mixer; full 36-layer solver"
echo "[slurm] fp16 AMP, fp32 weights/Adam/EMA; default batch 2 x 4 GPUs x accum4 = 32"
echo "[slurm] output=$RUN_OUT"
nvidia-smi -L
srun "$VENV/torchrun" --nproc_per_node 4 --master_port "$MASTER_PORT" \
    ...
    --config-name sft_cd_eos_2b_consistency2to1_2cam_nav_lcdrive "${EXTRA[@]}"